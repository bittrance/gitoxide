mod get {
    ...
    fn single_lines() -> crate::Result {
        ...
        let value = config.raw_values_mut("core.a")?;
...
c
   ...
   a"#
        ...
        let mut values = config.raw_values_mut("core.a")?;
        assert_eq!(
            &*values.get()?,
...
mod access {
    ...
    fn non_empty_sizes() -> crate::Result {
        let mut config = init_config();
        assert_eq!(config.raw_values_mut("core.a")?.len(), 3);
        assert!(!config.raw_values_mut("core.a")?.is_empty());
        Ok(())
    }
}
