...
[http]
  sslVerify = false",
    ...
    assert_eq!(config.string("core.c"), Some(cow_str("12")));
    assert_eq!(config.integer("core.d"), Some(Ok(41)));
    assert_eq!(config.boolean("http.sslVerify"), Some(Ok(false)));
    assert_eq!(config.boolean("diff.renames"), Some(Ok(true)));
...
fn respect_max_depth() -> crate::Result {
    ...
    let config = File::from_paths_metadata(into_meta(vec![dir.path().join("0")]), options)?.expect("non-empty");
    assert_eq!(config.integer("core.i"), Some(Ok(4)));
    ...
    let config = File::from_paths_metadata(into_meta(vec![dir.path().join("0")]), options)?.expect("non-empty");
    assert_eq!(config.integer("core.i"), Some(Ok(2)));
...
[core]
  b = false",
    ...
    assert_eq!(config.boolean("core.b"), Some(Ok(false)));
    Ok(())
}
...
[include]
  path = {}",
    ...
    assert_eq!(config.integers("core.b"), Some(Ok(vec![0, 1, 0, 1, 0])));
    Ok(())
}
...
[include]
  path = {}",
    ...
    assert_eq!(config.integer("core.c"), Some(Ok(1)));
