...
impl<'repo> SnapshotMut<'repo> {
    ...
    pub fn set_value<'b>(
        ...
        let current = match section.parent() {
            Some(parent) => self
                .config
                .set_raw_value(&format!("{}.{}.{}", parent.name(), section.name(), key.name()), value)?,
            None => self
                .config
                .set_raw_value(&format!("{}.{}", section.name(), key.name()), value)?,
        };
    ...
    pub fn set_subsection_value<'a, 'b>(
        ...
        let current = self.config.set_raw_value(&name, value)?;
        Ok(current.map(std::borrow::Cow::into_owned))
    }
